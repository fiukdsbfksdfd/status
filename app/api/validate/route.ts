...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: userRecord } = await supabase
      .from("users")
      .select("time_remaining, two_fa_enabled, two_fa_secret")
      .eq("id", user.id)
      .single()
    ...
    if (userRecord.two_fa_enabled) {
      if (!twoFactorCode) {
        return NextResponse.json({ status: "2fa_required" }, { status: 401 })
      }

      if (!userRecord.two_fa_secret || !(await verifyTOTP(userRecord.two_fa_secret, twoFactorCode))) {
        return NextResponse.json({ status: "invalid" }, { status: 401 })
      }
    }
